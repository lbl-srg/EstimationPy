...
from FmuUtils.CsvReader import CsvReader
...
class Test(unittest.TestCase):
    ...
    def test_measured_output(self):
        ...
        self.io_var.SetMeasuredOutput(True)
        self.assertTrue(self.io_var.IsMeasuredOutput(), "The flag measured output has been set to True but value does not correspond")
        
        self.io_var.SetMeasuredOutput(False)
        self.assertFalse(self.io_var.IsMeasuredOutput(), "The flag measured output has been set to True but value does not correspond")
    ...
    def test_set_covariance(self):
        ...
        """
        cov = -1.0
        self.assertFalse(self.io_var.SetCovariance(cov), "The covariance cannot be set to a negative value")
        self.assertNotEqual(cov, self.io_var.GetCovariance(), "The covariance shouldn't be set to the negative value")
        
        cov = 2.4
        self.assertTrue(self.io_var.SetCovariance(cov), "The covariance should be set to the value")
        self.assertEqual(cov, self.io_var.GetCovariance(), "The covariance should be set to the value")
    ...
    def test_set_object(self):
        ...
        self.assertRaises(TypeError, self.io_var.SetObject, None)
        
        # Create a dummy pyfmi.fmi.Scalarvariable and try to associate it to the InOutVar object
        v = pyfmi.fmi.ScalarVariable(0, 0, 0)
        self.io_var.SetObject(v)
        self.assertIsInstance(self.io_var.GetObject(), pyfmi.fmi.ScalarVariable, "The object returned by the GetObject method does not return a pyfmi.fmi.ScalarVariable object")
    ...
    def test_set_csv_reader(self):
        ...
        # Check that if trying to set the wrong type of object an exception is raised
        self.assertRaises(TypeError, self.io_var.SetCsvReader, None)
        
        # Associate to it a real csv reader and try to read it
        reader = CsvReader()
        self.io_var.SetCsvReader(reader)
        self.assertIsInstance(self.io_var.GetCsvReader(), CsvReader, "The object returned by the GetCsvReader method does not return a CsvReader object")
    ...
    def test_get_data_series_from_csv(self):
        ...
        reader = CsvReader()
        reader.OpenCSV(self.csvOK)
        self.io_var.SetCsvReader(reader)
        
        # Try to get a data series without specifying the selected column
        self.assertIsInstance(self.io_var.GetDataSeries(), pd.Series, "The reader has a CSV file assigned but not a column, the return value should be a pandas.Series")
        self.assertEqual(0, len(self.io_var.GetDataSeries()), "The reader has a CSV file assigned but not a column, the return value should be a pandas.Series empty")
        ...
        self.io_var.GetCsvReader().SetSelectedColumn(col_name)
        # Read the value from the csv file associated
        self.assertTrue(self.io_var.ReadDataSeries(), "The data series reda from the csv file is empty")
        
        # Now the dataSeries has been read, it is possible to get it
        self.assertTrue(numpy.allclose(data.values, self.io_var.GetDataSeries().values), "The pandas Series get is not equal to %s" % str(data.values))
        self.assertListEqual(data.index.tolist(), self.io_var.GetDataSeries().index.tolist(), "The index of the pandas Series get is not equal to %s" % str(data.index))
    ...
    def test_set_data_series(self):
        ...
        self.assertRaises(TypeError, self.io_var.SetDataSeries, None)
        self.assertRaises(TypeError, self.io_var.SetDataSeries, [])        
        self.assertRaises(TypeError, self.io_var.SetDataSeries, [1, 2, 3])
        
        # Trying to set a pandas.Series that has an integer index raises an exception
        s = pd.Series([1,2,3])
        self.assertRaises(TypeError, self.io_var.SetDataSeries, s)
        
        # Convert the index type to pandas.DatetimeIndex
        s.index = pd.to_datetime(s.index, unit="s")
        # Set the data series
        self.io_var.SetDataSeries(s)
        
        # Compare the values
        self.assertTrue(numpy.allclose(s.values, self.io_var.GetDataSeries().values), "The pandas Series get is not equal to %s" % str(s.values))
        self.assertListEqual(s.index.tolist(), self.io_var.GetDataSeries().index.tolist(), "The index of the pandas Series get is not equal to %s" % str(s.index))
    ...
    def test_read_from_data_series(self):
        ...
        for ix in s.index:
            self.assertEqual(s[ix], self.io_var.ReadFromDataSeries(ix), "The value read by the function ReadFromDataSeries does not correspond to the value set")
        ...
        for new_t in new_ts:
            ...
            int_v = self.io_var.ReadFromDataSeries(new_ix)
        ...
        for out_t in out_ts:
            ...
            self.assertFalse(self.io_var.ReadFromDataSeries(out_ix), "The index is out of range and the method ReadFromDataSeries has to return False")

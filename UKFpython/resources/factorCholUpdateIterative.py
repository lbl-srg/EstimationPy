...
n = 100
# True mean vector
Xtrue = array([[2.0, 3.0, 15.4, 21.3, 0.5, -8.35, 8.88, 34.5, 56.6, 21.9, 9]])

# Generate the sample for computing the covariance matrix
noUsed, N = Xtrue.shape
Xpoints = zeros((n,N))
for i in range(n):
	noise = random.uniform(-2.0,2.0,(1,N)) 
...
for x in Xpoints:
	...
	absW  = abs(Weights[i])
	signW = sign(Weights[i])
	X     = signW*sqrt(absW)*error[0]

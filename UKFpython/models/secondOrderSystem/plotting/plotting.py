...
def plotResults(time,stopTime,X,Y,Z,Xhat,Yhat,P,CovZ):
	# presentation of the results
	fig = plt.figure()
	ax  = fig.add_subplot(411)
	ax.plot(time,X[:,0],'b',label='$x$')
	ax.plot(time,Xhat[:,0],'g',label='$x_{UKF}$')
	ax.fill_between(time, Xhat[:,0]-np.sqrt(P[:,0,0]), Xhat[:,0] +np.sqrt(P[:,0,0]), facecolor='green', interpolate=True, alpha=0.5)
